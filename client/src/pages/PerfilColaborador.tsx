...
function saldoBg(min: number) {
  if (min > 0) return "bg-emerald-50 border-emerald-200";
  if (min < 0) return "bg-red-50 border-red-200";
...
function LinhaRegisto({ r }: { r: any }) {
  ...
  function cellClass(auto: boolean) {
    return `font-mono text-xs px-2 py-1 rounded ${
      auto
  ...
  return (
    <tr className={`border-b border-border hover:bg-muted/40 transition-colors ${isJust ? "opacity-60" : ""}`}>
      <td className="px-3 py-2 text-xs text-muted-foreground whitespace-nowrap">
        {r.data?.slice(0, 5)}
        <span className="ml-1.5 text-muted-foreground/60 uppercase text-[10px]">{r.diaSemana}</span>
      </td>
      <td className="px-2 py-2">
        {isJust ? (
          <span className="text-xs text-blue-600 italic">{r.justificacao}</span>
        ) : (
      ...
      <td className="px-3 py-2 text-xs text-muted-foreground max-w-[200px] truncate hidden md:table-cell">
        {r.detalhe}
      </td>
    </tr>
  );
...
function BlocoMes({ m, numero }: { m: any; numero: string }) {
  ...
  return (
    <div className={`rounded-xl border transition-all ${saldoBg(m.saldoTotal)}`}>
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <Link
              ...
              className="text-xs text-primary hover:underline flex items-center gap-1 mt-0.5 transition-colors"
            >
      ...
      {aberto && (
        <div className="border-t border-border overflow-x-auto bg-card rounded-b-xl">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-3 py-2 text-left text-xs text-muted-foreground font-medium">Data</th>
                <th className="px-2 py-2 text-left text-xs text-muted-foreground font-medium">Picagens</th>
                <th className="px-3 py-2 text-right text-xs text-muted-foreground font-medium">Saldo</th>
                <th className="px-3 py-2 text-left text-xs text-muted-foreground font-medium hidden md:table-cell">Detalhe</th>
              </tr>
            </thead>
            <tbody>
          ...
          {/* Legenda */}
          <div className="px-4 py-2 flex gap-4 text-[10px] text-muted-foreground border-t border-border">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded bg-amber-300" /> Preenchido automaticamente
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded bg-blue-300" /> Justificado
            </span>
          </div>
        </div>
      )}
...
export default function PerfilColaborador() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center space-y-3">
          ...
          <p className="text-foreground">Colaborador não encontrado</p>
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      {/* Cabeçalho do perfil */}
      <div className="flex items-start gap-4 p-5 rounded-2xl bg-card border border-border shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold text-foreground truncate">{nome}</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Nº {numero} · {historico.length} {historico.length === 1 ? "mês" : "meses"} registados</p>
        </div>
        {/* Saldo acumulado total */}
        <div className={`text-right px-4 py-2 rounded-xl border ${saldoBg(totais.saldoTotal)}`}>
          <p className="text-xs text-muted-foreground mb-0.5">Saldo acumulado</p>
        ...
        {[
          ...
          <div key={c.label} className="bg-card border border-border rounded-xl p-4 shadow-sm">
            ...
            <p className={`font-mono font-bold text-lg ${c.color}`}>
              {c.show ? c.value : <span className="text-muted-foreground">—</span>}
            </p>
          </div>
        ))}
      </div>
      ...
      {dadosGrafico.length > 0 && (
        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-foreground">Evolução do Saldo Mensal</h2>
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-foreground">Histórico por Mês</h2>
          <span className="text-xs text-muted-foreground ml-1">— clique para expandir os registos diários</span>
        </div>
